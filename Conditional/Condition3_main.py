...
def day_kiwoom_db3(self, code=None, date=None, sPrevNext='0'):
    QTest.qWait(700)  # 3.6초의 딜레이, 1시간에 1000개까지만 조회 및 주문가능 (1시간은 3600초따라서 3.6초가 1000개 달성)
...
def trdata_slot3(self, sCrNo, sRQName, sTrCode, sRecordName, sPrevNext):
    ...
    if sRQName == "주식일봉차트조회":
        ...
        if cnt < 40:  # 5일씩 1개월에 4주로 2개월치
            ...
            print("종목 이름: %s" % self.calcul_data[0][8])
            ...
            # 비교를 위한 초기값 생성
            sj_0 = self.calcul_data[0][1] - self.calcul_data[0][5]  # 초기 변수화 (현재가는 종가 - 그날 하루의 시작가인 시가)

            if self.calcul_data[0][6] - self.calcul_data[0][7] == 0:
                print("최근일자부터 거래 정지여? 뭐여?")
                self.calculator_event_loop.exit()

            elif self.calcul_data[0][1] - self.calcul_data[0][5] == 0:
                sj_0 = self.calcul_data[0][6] - self.calcul_data[0][7]
                af_day_v = (self.calcul_data[0][6] - self.calcul_data[0][7]) / sj_0

            else:
                af_day_v = (self.calcul_data[0][6] - self.calcul_data[0][7]) / (self.calcul_data[0][1] - self.calcul_data[0][5])  # 비교값 초기 생성


            ####################
            # 본격적으로 조건문 시작
            for idx in range(40):
                sj = self.calcul_data[idx][1] - self.calcul_data[idx][5]  # 종가-시가를 변수화

                if self.calcul_data[idx][6] - self.calcul_data[idx][7] == 0:  # 고가와 저가가 같으면 거래정지
                    print("거래정지인듯 ㅋㅋ \n")
                    break

                if self.calcul_data[idx][1] - self.calcul_data[idx][5] == 0:
                    sj = (self.calcul_data[idx][6] - self.calcul_data[idx][7])  # 시가-종가가 0이면 지표가 1이 되도록 수정

                if self.kos_mdf_data[idx]*10 < 7:
                    self.kos_mdf_data[idx] = 0.7  # 코스 시장의 이동일변동이 5보다 작으면 5로 수정!!!, 여기서 종목의 이동변동의 하한성을 선정!!!

                else:
                    day_v = (self.calcul_data[idx][6] - self.calcul_data[idx][7]) / sj
                    # 일일변동추정 지표: "(고가-저가)/(종가-시가)"

                    # 변동성에 따른 조건
                    if day_v > 7:  # 7로 잡음
                        print("%s일, 일일변동 높음, 일일변동성: %s \n" % (idx, day_v))   # 여기서 일일변동의 제한을 검!!!!
                        break

                    if (day_v - af_day_v) > (self.kos_mdf_data[idx]*10):  # 코스피의 이동일변동
                        print("이동일변동 허용치 %s" % (self.kos_mdf_data[idx]*10))
                        print("%s일, 이동일변동 높음, 이동일변동: %s \n" % (idx, (day_v - af_day_v)))
                        break
                    ...
                    if day_v <= 7 and (day_v - af_day_v) <= (self.kos_mdf_data[idx]*10) and idx == 39:
                        print("해당 종목이 조건 통과!! \n")

                        notify_stock_signal("DFEI 조건항목: %s" % code_nm)
                        append_condition_result("condition3_stock_0208.txt", code, code_nm)
                        break
